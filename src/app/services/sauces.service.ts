...
export class SaucesService {
  ...
  getSauces() {
    this.http.get<Sauce[]>(`${apiUrl}/api/sauces`).pipe(
  ...
  getSauceById(id: string) {
    return this.http.get<Sauce>(`${apiUrl}/api/sauces/` + id).pipe(
  ...
  createSauce(sauce: Sauce, image: File) {
    ...
    return this.http.post<{ message: string }>(`${apiUrl}/api/sauces`, formData).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      return this.http.put<{ message: string }>(`${apiUrl}/api/sauces/` + id, sauce).pipe(
        catchError(error => throwError(error.error.message))
      );
    } else {
      const formData = new FormData();
      formData.append('sauce', JSON.stringify(sauce));
      formData.append('image', image);
      return this.http.put<{ message: string }>(`${apiUrl}/api/sauces/` + id, formData).pipe(
        catchError(error => throwError(error.error.message))
      );
    }
  }
  ...
  deleteSauce(id: string) {
    return this.http.delete<{ message: string }>(`${apiUrl}/api/sauces/` + id).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
}
